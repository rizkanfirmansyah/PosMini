                        ...
                        <div class="card">
                            ...
                            <div class="card-body p-3">
                                <div class="table-responsive">
                                    <table class="table m-0" id="table-supplier">
                                        <thead>
                                            <tr>
                                                <th>Nama</th>
                                                <th>Alamat</th>
        ...
        $('form').on('submit', function(e) {
            ...
            if (id == undefined) {
                data = {
                    ...
                    url: '/api/v1/suppliers/store',
                    validation: true
                }
            } else {
                data = {
                    data: new FormData(this),
                    url: '/api/v1/suppliers/update/'+id,
                    validation: true
                }
            }
            crud.input(data);
            crud.reload('table-supplier')
        })

        $('#table-supplier').on('click', '.Delete', async function() {
            let id = $(this).data('id')
            let data = {
                data: id,
                url: '/api/v1/suppliers/destroy',
                table: 'table-supplier'
            }
            crud.delete(data)
        })

        $('#table-supplier').on('click', '.Edit', async function() {
            let id = $(this).data('id')
            let data = {
                data: id,
                url: '/api/v1/suppliers/edit',
            }
            ...
            if (response) {
                ...
                $('#EditData').data('id', response.data.supplier.id);
            }
        })
